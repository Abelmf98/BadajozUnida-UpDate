...

{
  ...
  */
  ...
  /**
   * @param $datos
   * @return int Devuelve el id de la última ubicación insertada
   */
  function guardarCoordenadas($datos)
  {

    $datosPorDefecto = "No hay informacion temporalmente";

    if ($datos['idUbicacion'] == "") {
      $datos['idUbicacion'] = [31.2785, -7.1];
    }

    $query = "INSERT INTO Ubicacion (nombre, direccion, latitud, longitud) 
    VALUES ('" . $datosPorDefecto . "', '" . $datosPorDefecto . "', " . $datos['idUbicacion'][0] . ", " . $datos['idUbicacion'][1] . ")";

    $resul = parent::nonQueryId($query, true);
